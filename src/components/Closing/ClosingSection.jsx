  ...
  const observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        // Trigger ONLY when she has scrolled past 80% of the letter
        if (entry.isIntersecting && entry.intersectionRatio >= 0.8 && !triggeredRef.current) {
          triggeredRef.current = true;
          onReachEnd?.();
        }
      });
    },
    { threshold: 0.8 } // Higher threshold = wait until the end of the section
  );
